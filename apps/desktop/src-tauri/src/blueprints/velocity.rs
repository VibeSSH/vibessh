...
use super::{JAVA_PATH_KEY, chosen_server_jar, with_forced_ansi, ensure_java_for, render_java_config, text_input, text_list_input, validate_inputs, BlueprintHandler, ProvisionContext};
...
impl BlueprintHandler for VelocityBlueprint {
    ...
    fn render_runtime_config(&self, inputs: &HashMap<String, serde_json::Value>) -> AppResult<serde_json::Value> {
        ...
        render_java_config(&java_version, jvm_args, jar, program_args, java_path, Some("end"))
    }
...
mod tests {
    ...
    fn render_runtime_config_builds_a_docker_image_and_command_with_the_downloaded_jar() {
        ...
        assert_eq!(config, serde_json::json!({ "image": "eclipse-temurin:21-jre", "command": ["java", "-Dterminal.ansi=true", "-jar", "velocity-3.4.0-566.jar"], "runAsDedicatedUser": true }));
    }
    ...
    fn render_runtime_config_places_jvm_args_before_jar_and_program_args_after() {
        ...
        assert_eq!(config["command"], serde_json::json!(["java", "-Dterminal.ansi=true", "-Xmx1G", "-jar", "velocity-3.4.0-566.jar", "--example"]));
    }
