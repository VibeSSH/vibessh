...
/**
 * Ported near-verbatim from Voltius's own useRipple hook (voltius/src/hooks/
 * useRipple.tsx) - a pure UI mechanic with zero coupling to Voltius-specific
 * concepts, safe to reuse as-is. Every button in their app wires this to
 * onPointerDown; it's a big part of why their interface reads as premium/
 * tactile rather than flat, and VibeSSH had nothing like it before this.
 */
interface Ripple {
  id: number;
  x: number;
...
export function useRipple() {
  ...
  const createRipple = useCallback((e: React.PointerEvent<HTMLElement>) => {
    ...
    // Capture on the button: a press-time re-render can detach the icon
    // node from under the pointer, which would otherwise drop the release.
