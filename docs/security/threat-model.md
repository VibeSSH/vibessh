...
| On-path network attacker | **No, after the first connection** | TOFU pinning on both SSH and Agent transports, which detects a key that changes and cannot detect one that was wrong to begin with. A server that gains a second kind of host key (an ECDSA key beside its RSA one) is not treated as changed: the connection asks for each other kind in turn and goes ahead only if one of them is the recorded key, which the handshake proves by the server signing with it. A key that really changed still gets the full warning, with both fingerprints and the command to compare them on the Node. Somebody positioned on the path during the very first connection to a Node is trusted at that moment and pinned thereafter. Closing that needs a fingerprint checked out of band, which the interface shows and does not require. |
